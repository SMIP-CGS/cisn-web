...
function getRegression (obsArr, newPlot, regrType) {
  $.getJSON(
    './data/' + eventid + '/attenuation_curves.json',
    function(json) {
      var regrPoints = json;
  ...
  function return_data(regrPoints, obsArr) {
    ...
    var distance_max = Math.max(...obsArr.map(o => o.distance), 1);
    ...
    for (var i=0; i<regrPoints['distances']['repi'].length; i++) {
      if (regrPoints['distances']['repi'][i]  < 301
            ...
            && regrPoints['distances']['repi'][i] < distance_max
...
function stationList(newPlot, regrType, showDYFI) {
  ...
  function return_data(stations) {
    ...
    for (var i=0; i<stations.length; i++) {
      ...
      if(fileExists('./data/' + eventid + '/attenuation_curves.json')) {
