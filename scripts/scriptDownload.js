...
function listProducts (eventid, productList) {
  ...
  var baseLink = 'data/' + eventid +'/';
  var myvar =
    '<table class="table table-hover table-sm archive_table">' +
    '<thead>' +
    '<tr class="table-dark">' +
    '<th style="text-align:left;color:black;" scope="col";>File name</th>' +
    '<th style="text-align:left;color:black;" scope="col";>Description</th>' +
    '</thead>' +
    '</tr>' +
    '<tbody>';
  ...
  for (var k = 0; k < catList.length; k++) {
    if (categories.indexOf(catList[k]) >= 0) {
      myvar += '<tr data-href="#"><td colspan = "100%" bgcolor="#BA5711" style="text-align:left"><font size="3" color="#ffffff"><b>' + catList[k] +
       '</b></font></td></tr>';
      for (var i = 0; i < productsNumber; i++) {
        if (productsList[i].cat === catList[k]) {

          if (productList[i].name.slice(-5) === '.jpg' && catList[k]==='Peak Ground Motion Maps' && mapType===0) {
            myvar += '<tr data-href="#"><td colspan = "100%" bgcolor="#BA5711" style="text-align:left"><font size="2" color="#ffffff"><b> JPG maps </b></font></td></tr>';
            mapType = 1;
          }
          if (productList[i].name.slice(-5) === '.pdf' && catList[k]==='Peak Ground Motion Maps' && mapType===1) {
            myvar += '<tr data-href="#"><td colspan = "100%" bgcolor="#BA5711" style="text-align:left"><font size="2" color="#ffffff"><b> PDF maps </b></font></td></tr>';
            mapType = 2;
          }

          myvar += '<tr tabindex="0" data-href="' + baseLink + productsList[i].name + '">' +
...
var productsList;
$.getJSON('data/' + eventid + '/productList.json', function (data) {
  productsList = data;
