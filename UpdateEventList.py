...
def overlay_to_json(event_id): 
    info_file_path = './data/' + event_id + '/intensity_overlay.pngw'
    with open(info_file_path, 'r', encoding='utf-8') as overlay:
    ...
    with open('./data/' + event_id + '/overlay.json', 'w', encoding='utf-8') as outfile:
...
def get_bBox_dict():
    if (os.path.isfile('bBox.txt')):
        with open('bBox.txt', encoding='utf-8') as json_file:
...
def get_products_list(event_id):
    ...
    """
    with open('productsDownloadList.json', encoding='utf-8') as json_file:
        productMeta = json.load(json_file)

    products_path = './data/' + event_id + '/'
    ...
    with open(products_path + 'productList.json', 'w', encoding='utf-8') as outfile:
...
def get_parameters (event_id, bBox):
    ...
    """
    info_file_path = './data/' + event_id + '/info.json'
    ...
    parameter_dict = {
            'id': event_id,
            'description': info_file['input']['event_information']['location'],
            'day': day,
            'month': month,
            'year': year,
            'hour': hour,
...
def write_list_to_file(event_list):
    ...
    with open('events.js.tmp', 'w', encoding='utf-8') as f:
        print('var events =', file=f)
    with open('events.js.tmp', 'a', encoding='utf-8') as outfile:
...
def write_version_file():
    ...
    with open(yaml_file_path, 'r', encoding='utf-8') as yaml:
    ...
    with open('./scripts/softwareVersion.js', 'w', encoding='utf-8') as f:
...
def do_for_all_events(bBox):
    ...
    for event_del in folders_list:
        delete_event1(event_del)
    folders_list = get_event_ids()    
...
def update_event_list(eventParameters, event_id, eventAction='add'):
    if os.path.isfile('events.js'):
        ...
        with open('events.js.tmp', 'r', encoding='utf-8') as f:
...
def delete_event1(event_id):
    if os.path.exists('./data/' + event_id + '/cancel.txt'):
        try:
            shutil.rmtree('./data/' + event_id)
            print(event_id)
            print("folder removed from data")
        except Exception as e:
            print('Could not delete folder for event: ' + event_id + ':')
            print(e)
    else:
        print('No cancel.txt' + event_id + ':')
